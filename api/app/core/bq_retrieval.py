...
class BQRetrievalService:
    ...
    def clear(self, mode: RetrievalModeV2 | None = None) -> None:
        ...
        """
        if mode is None or mode == RetrievalModeV2.BINARY:
            if self._milvus_store:
                self._milvus_store.clear()
                self._milvus_initialized = False
        
        if mode is None or mode == RetrievalModeV2.FLOAT32:
            if self._float32_engine and hasattr(self._float32_engine, 'clear'):
