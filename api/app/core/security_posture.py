...
def build_security_posture() -> SecurityPostureResponse:
    ...
    checks.append(SecurityPostureCheck(
        id="docs",
        status="pass",
        message="Interactive API docs are blocked in exposed mode" if exposed_mode else "Interactive API docs are local-only",
    ...
    if not recommendations:
        recommendations.append("Keep API keys scoped to client installs and rotate them between engagements.")
