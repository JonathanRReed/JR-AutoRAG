...
if TYPE_CHECKING:
    from .hierarchy import DocumentTree, HierarchyBuilder, HierarchicalRetriever
...
class AdvancedRetriever:
    ...
    def _detect_optimal_mode(self, query: str) -> AdvancedRetrievalMode:
        ...
        if any(p in query_lower for p in hierarchical_patterns):
            if self._document_trees:
                return AdvancedRetrievalMode.RAPTOR
    ...
    def _apply_graph_retrieval(
        ...
        for community in self._graph_rag.communities:
            if any(ent in community.entities for ent in entity_names):
