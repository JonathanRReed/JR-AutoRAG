...
def test_get_chunker_recursive():
    ...
    chunker = get_chunker(strategy=ChunkingStrategy.RECURSIVE, target_size=600, overlap=20)
...
def test_get_chunker_semantic():
    ...
    chunker = get_chunker(strategy=ChunkingStrategy.SEMANTIC, target_size=800, overlap=100)
    ...
    chunker_str = get_chunker(strategy="semantic", min_size=50, similarity_threshold=0.8)
