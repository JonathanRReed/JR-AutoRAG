...
export function TypingAnimation({
  ...
  useEffect(() => {
    ...
    if (!shouldReduceMotion && shouldStart && wordsToAnimate.length > 0) {
      const timeoutDelay =
  ...
  const getCursorChar = () => {
    return cursorStyle === "underscore" ? "_" : "|"
  }
  ...
  return (
    <MotionComponent
      ...
      {...props}
    >
      {shouldReduceMotion ? children ?? words?.[0] ?? "" : displayedText}
      {!shouldReduceMotion && shouldShowCursor && (
        <span
