...
export function EnterpriseControls({ className, onViewArtifact }: EnterpriseControlsProps) {
    ...
    const StatusBadge = ({ state }: { state?: ArtifactState }) => {
        if (!state || state.status === "not_built") {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/5 px-2 py-0.5 text-[10px] text-muted-foreground">
        ...
        if (state.status === "building") {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] text-amber-400 ring-1 ring-inset ring-amber-500/20">
        ...
        if (state.status === "failed") {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-red-500/10 px-2 py-0.5 text-[10px] text-red-400 ring-1 ring-inset ring-red-500/20" title={state.error}>
    ...
    return (
        <div className={`mt-4 rounded-xl border border-white/10 bg-[#0f1117] p-4 shadow-sm ${className}`}>
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <Database className="h-4 w-4 text-indigo-300" />
                    <span className="text-sm font-medium text-indigo-100">Artifact Builds</span>
                </div>
                <span className="text-[10px] font-mono text-indigo-200/40">G4</span>
            </div>
            ...
            <div className="space-y-2">
                ...
                <div
                    ...
                    className={`group flex items-center justify-between rounded-lg bg-black/20 p-2 transition-all ${status?.graph_rag.status !== "not_built" ? "cursor-pointer hover:bg-black/40" : "opacity-50"}`}
                >
                    <div className="flex items-center gap-2">
                        <Network className="h-3.5 w-3.5 text-violet-400" />
                        <div>
                            <p className="text-xs font-medium text-indigo-100">GraphRAG</p>
                        </div>
                    </div>
                ...
                <div
                    ...
                    className={`group flex items-center justify-between rounded-lg bg-black/20 p-2 transition-all ${status?.raptor.status !== "not_built" ? "cursor-pointer hover:bg-black/40" : "opacity-50"}`}
                >
                    <div className="flex items-center gap-2">
                        <Layers className="h-3.5 w-3.5 text-cyan-400" />
                        <div>
                            <p className="text-xs font-medium text-indigo-100">RAPTOR</p>
                        </div>
                    </div>
